...
print("=== ZIP members (media/images) ===")
for n in z.namelist():
...
for row in rows[1:]:
    ...
    print(f"--- {src} | flags={flags} | col5(ch)={repr(cells.get(5))[:18]} col7(diff)={cells.get(7)} col8(marks)={cells.get(8)} | fig={figtxt} dispimg={has_disp}")
    print("    stem:", repr(s[:280]))

print("=== cellimages.xml ===")
if "xl/cellimages.xml" in z.namelist():
    print(z.read("xl/cellimages.xml").decode("utf-8", "replace")[:1500])
