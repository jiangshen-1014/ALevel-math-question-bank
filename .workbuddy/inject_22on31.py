...
XLSX = r"F:/题库输入表格/CIE P3 录入/22_ON_31.xlsx"
...
# 按 qno 推断章节（本文件章节列填的是无意义数字占位：Q9='10' Q11='11'）。
CHAPTER = {
    1:  ["Algebra (modulus functions)"],                           # 模函数图 + 含绝对值不等式
    2:  ["Complex numbers"],                                       # Argand 图 shading 区域
    3:  ["Logarithmic and exponential functions"],                 # 指数方程用 ln 表示
    4:  ["Trigonometry"],                                          # 三角方程
    5:  ["Complex numbers"],                                       # 复数指数形式运算 + 最小正整数 n
    6:  ["Trigonometry"],                                          # 三角恒等式 + 解三角方程
    7:  ["Differentiation", "Numerical solution of equations"],    # 由梯度=12 导驻点方程，迭代定位 a
    8:  ["Differential equations"],                                # 微分方程 dx/dt = kxe^{-0.1t}
    9:  ["Differentiation", "Integration"],                        # 求曲线最小点 + 面积（有图）
    10: ["Algebra (partial fractions and binomial expansions)"],   # 部分分式 + 二项展开
    11: ["Vectors"],                                               # 向量固体图（有图）
}
DIFF = {1:3, 2:3, 3:3, 4:4, 5:4, 6:4, 7:4, 8:4, 9:4, 10:4, 11:4}
...
# DISPIMG ID -> media file（兼容 WPS <etc:cellImage> 与 <xdr:pic> 两种结构）
