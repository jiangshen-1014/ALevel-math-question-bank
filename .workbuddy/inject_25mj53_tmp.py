...
# ===================== 需填写 =====================
XLSX = r"F:/题库输入表格/CIE S1录入/25_MJ_53.xlsx"       # TODO: 填实际 S1 录入表
APPLY = True                                             # 确认无误后改 True 才会写入 data.js
# =================================================
...
CHAPTER = {
    1: ["Representation of data（数据表示）"],                                        # 40 个 x 值：编码法求 mean & sd
    2: ["Probability（概率）"],                                                       # 二项概率 X~B(3,0.45) / Y~B(10,0.55)
    3: ["Discrete random variables（离散随机变量）"],                                # 几何分布 P(X=k) 直到掷出 3
    4: ["Representation of data（数据表示）"],                                        # 累积频数图读图估计区间人数
    5: ["Probability（概率）"],                                                       # 两袋弹珠树状图概率
    6: ["The normal distribution（正态分布）"],                                       # 正态求概率 + 期望数量(160×p) + 逆查 μ,σ
    7: ["Permutations & combinations（排列组合）"],                                   # 7 男 4 女排列、兄弟不相邻
}
# 每题难度 1–5；留空 {} 则按分值推断
DIFF = {
    1: 3, 2: 4, 3: 4, 4: 4, 5: 4, 6: 5, 7: 5,
}
