...
# ---- image ID -> media mapping (cellimages.xml + rels) ----
img_id_to_media = {}
if "xl/cellimages.xml" in z.namelist():
    ...
    for img in ci.iter(f"{A}picture"):
        # nvPicPr -> cNvPr id; blip r:embed -> rel
...
# also parse drawing1/2.xml which may hold DISPIMG anchors with cell refs
def parse_drawing(dname):
    if dname not in z.namelist():
        return {}
    d = ET.fromstring(z.read(dname))
    out = {}
    for anchor in d.iter():
        # find marker with from col/row
        frm = anchor.find(f"{NS}from") if anchor.tag.endswith("from") else None
        # crude: look for editAs / object with DISPIMG? simpler: scan xml text for DISPIMG + cell
        pass
    return out

# The figure cell contains =DISPIMG("ID_xxx",1). Extract which DISPIMG IDs appear and map.
# We'll scan all sheet cells for DISPIMG and resolve.
...
for row in rows[1:]:
    ...
    for c in sorted(cells):
        ...
        print(f"    col{c} [{flags}]: {repr(sv[:90])}{extra}")
