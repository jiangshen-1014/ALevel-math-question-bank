...
# ===================== 需填写 =====================
XLSX = r"F:/题库输入表格/CIE S1录入/25_MJ_55.xlsx"       # TODO: 填实际 S1 录入表
...
CHAPTER = {
    1: ["Discrete random variables（离散随机变量）"],                                  # 两骰子差值 X：分布表 + E(X),Var(X)
    2: ["The normal distribution（正态分布）"],                                       # 树高正态：期望数量(<14m) + 逆查 h(25% tall)
    3: ["Discrete random variables（离散随机变量）", "The normal distribution（正态分布）"],  # 二项分布：正态近似(<27) + 精确二项(≤2)
    4: ["Probability（概率）"],                                                       # 入学考试树状图：接受概率 + 条件概率 + 三人均拒
    5: ["Representation of data（数据表示）"],                                        # 背靠背茎叶图 + 箱线图 + 比较
    6: ["Probability（概率）", "Permutations & combinations（排列组合）"],             # 组合概率(1男3女) + 分队 ways + 排列(George-Harry)
}
# 每题难度 1–5；留空 {} 则按分值推断
DIFF = {
    1: 3, 2: 4, 3: 4, 4: 4, 5: 4, 6: 5,
}
