...
XLSX = r"F:/题库输入表格/CIE P3 录入/22_MJ_33.xlsx"
...
# 按 qno 推断章节（本文件章节列全空）。
CHAPTER = {
    1:  ["Algebra (modulus functions)"],                           # 含绝对值不等式 |2x+a|<|2x+3a|
    2:  ["Trigonometry"],                                          # 三角方程
    3:  ["Logarithmic and exponential functions"],                 # log 方程 -> 二次
    4:  ["Differentiation"],                                       # 曲线驻点（dy/dx=0）
    5:  ["Complex numbers"],                                       # 复数 Argand 图 + 共轭
    6:  ["Differentiation"],                                       # 参数方程 dy/dx + 切线（纯微分法）
    7:  ["Algebra (partial fractions and binomial expansions)"],   # 部分分式 + 二项展开
    8:  ["Differential equations"],                                # 微分方程
    9:  ["Vectors"],                                               # 向量（线+垂足+反射）
    10: ["Integration", "Numerical solution of equations"],        # 积分方程导出 a + 迭代定位（有图占位但无嵌入）
}
DIFF = {1:3, 2:4, 3:3, 4:4, 5:4, 6:4, 7:4, 8:4, 9:4, 10:4}
...
# sharedStrings（本文件为内联字符串，无 sharedStrings）
shared = []
if "xl/sharedStrings.xml" in names:
    for si in ET.fromstring(z.read("xl/sharedStrings.xml")).findall(f"{NS}si"):
        shared.append("".join(t.text or "" for t in si.iter(f"{NS}t")))

# DISPIMG 映射（本文件无 cellimages，DISPIMG_MEDIA 为空）
DISPIMG_MEDIA = {}
...
for r in sorted(rows):
    ...
    # 配图：仅 DISPIMG 提取；数字占位/空/无 cellimages → 留空
    figure = ""
